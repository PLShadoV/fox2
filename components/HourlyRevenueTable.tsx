export default function HourlyRevenueTable({ rows }: { rows: Array<{hour:number;kwh:number;price_pln_mwh:number;price_used_pln_mwh:number;revenue_pln:number;}> }) {
  ...
  return (
    <div className="overflow-auto rounded-2xl border border-zinc-200/60 dark:border-zinc-800 bg-white/60 dark:bg-zinc-900/60 backdrop-blur">
      <table className="w-full text-sm">
        <thead className="sticky top-0 bg-zinc-50/80 dark:bg-zinc-800/80 backdrop-blur">
          <tr>
        ...
        <tbody>
          {rows.map(r=> (
            <tr key={r.hour} className="border-t border-zinc-100 dark:border-zinc-800">
        ...
        <tfoot>
          <tr className="border-t-2 border-zinc-200 dark:border-zinc-700 bg-zinc-50/60 dark:bg-zinc-800/40">
