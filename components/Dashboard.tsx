...
export default function Dashboard({ date }: { date: string }){
  ...
  const [exportKWh, setExportKWh] = useState<number[]>(new Array(24).fill(0));
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      setError(null);
      try {
        const [foxRes, rceRes, rtRes] = await Promise.all([
          fetch(`/api/foxess?date=${date}`),
        ...
        if (!cancelled) {
          setExportKWh(Array.isArray(fox?.exportKWh) ? fox.exportKWh : new Array(24).fill(0));
  ...
  const totalKWh = useMemo(()=> hourly.reduce((a,b)=> a + b.kwh, 0), [hourly]);
